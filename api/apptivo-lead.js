...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rentalDays = Number(String(b.rentalDays || "").replace(",", "."));
    const jobLocation = String(b.jobLocation || "").trim();
    const deliveryShift = String(b.deliveryShift || "").trim();
    ...
    if (!rentalDays || rentalDays <= 0 || !jobLocation || !deliveryShift) {
      return res.status(400).json({ ok: false, error: "Faltan campos obligatorios del arriendo (días/lugar/horario)." });
    }
